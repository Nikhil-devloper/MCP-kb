...
// Interface for document with embedding
...
async function storeInOpenSearch(document: DocumentWithEmbedding): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (!createError.message?.includes('resource_already_exists_exception')) {
...
async function directOpenSearchIndex(document: DocumentWithEmbedding): Promise<void> {
  if (!OPENSEARCH_ENABLED) {
  ...
  try {
    ...
    console.error('Error with direct HTTP indexing:', error?.message);
...
async function verifyDocumentIndexed(documentId: string, parsedUrl: URL, credentials: any): Promise<void> {
...
async function processRecord(record: DynamoDBRecord): Promise<void> {
  ...
  try {
    ...
    const document = unmarshall(record.dynamodb.NewImage as Record<string, AttributeValue>) as Document;
