...
async function getLatestLogStream(logGroupName: string) {
  try {
...
async function getLogEvents(logGroupName: string, logStreamName: string) {
  try {
...
function displayLogEvents(logGroupName: string, events: any[]) {
  console.log(`\n===== ${logGroupName} =====`);
  if (events.length === 0) {
